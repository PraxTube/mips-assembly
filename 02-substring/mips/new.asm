...
count:
  ...
  move $s0, $a0         # Save text string in $s0
  move $s1, $a1         # Save part string in $s1
  move $s2, $zero       # Initialize counter to 0


init_part:
  li $a0, 0x76543210    # Initialize $a0 to init hash
  move $a2, $zero       # Initialize $a2 (out char) to 0 (treated as no char)
  move $s6, $zero       # Initialize length of part $s6 to 0
...
init_text:
  li $a0, 0x76543210    # Initialize $a0 to init hash
  move $a2, $zero       # Initialize $a2 (out char) to 0 (treated as no char)
  move $s4, $zero       # Initialize the current length of our window to 0

init_text_loop:
  lb $a1, 0($s0)        # Load next char into $a1 (in char)
  beq $s4, $s6, end_init_text_loop # End loop if we have the length of part

  addi $s0, $s0, 1      # Move to next character in string
  addi $s4, $s4, 1      # Increment current window length
  jal rollhash
  move $a0, $v0         # Move newly calculated hash to $a0 (input for next calculation)
  j init_text_loop

end_init_text_loop:
  move $a0, $v0         # Move result hash into $a0 as we will need it as input for next rollhash


compare_loop:
  # Compare the current hash with the part hash and increment counter if equal
  beq $a0, $s1, not_increment_counter
...
not_increment_counter:
  ...
  jal rollhash
  move $a0, $v0         # Move newly calculated hash to $a0 (input hash for next rollhash)
  j compare_loop

end:
  move $v0, $s2         # Finally move counter to $v0
	jr $s7                # Return to saved $ra of caller main
